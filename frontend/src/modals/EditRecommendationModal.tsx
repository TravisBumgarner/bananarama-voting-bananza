...
import { Heading, Button, LabelAndInput, Paragraph, Checkbox, ButtonWrapper } from 'sharedComponents'
import { TRecommendation } from 'types'
import { context } from 'context'
import { colors } from 'theme'
import { logger } from 'utilities'

const EDIT_RECOMMENDATION = gql`
...
const RecommendationModal = ({ closeModal, recommendation }: RecommendationModalProps) => {
    const [editRecommendation] = useMutation<{ editRecommendation: TRecommendation }>(EDIT_RECOMMENDATION)

    const [latitude, setLatitude] = useState<number | null>(recommendation.latitude)
    const [longitude, setLongitude] = useState<number | null>(recommendation.longitude)
    const [location, setLocation] = useState<string>(recommendation.location)
    const [notes, setNotes] = useState<string>(recommendation.notes)
    const [recommender, setRecommender] = useState<string>(recommendation.recommender)
    const [hasVisited, setHasVisited] = useState<boolean>(recommendation.hasVisited)
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        setIsLoading(true)
        const editedRecommendation: TRecommendation = {
        ...
        try {
            const response = await editRecommendation({
                variables: editedRecommendation,
            })
            if (!response.data || response.data.editRecommendation === null) {
    ...
    return (
        <div>
            <div>
                ...
                <form onChange={() => setHasUserEdited(true)} onSubmit={handleSubmit}>
                    {!isEditingLocation
                        ? (
                            <Paragraph>
                    ...
                    <Checkbox
                        label="Have you visited?"
                        checked={hasVisited}
                        toggleChecked={() => setHasVisited((prev) => !prev)}
                    />
                    <ButtonWrapper
                        right={[
                            <Button key="cancel" onClick={closeModal} disabled={isLoading} variation="warning">Cancel</Button>,
                            <Button key="submit" type="submit" disabled={isLoading || !hasUserEdited} variation="primary">Submit</Button>
                        ]}
                    />
                </form>
            </div>
